...
export default class RestPW extends Component {
        ...
        return(
            <View style={style.root}>
                ...
                <View style={style.content}>
                    ...
                    <Text style={style.messages}> {this.state.messages[this.state.tabIndex]} </Text>

                    {/* 1.4 탭화면 구성 */}
                    <View style={style.tabContainer}>
                        {
                            this.state.tabs.map((value, index)=>{
                                return <TabComponents onPress={()=>this.setState({tabIndex:index})} selected={index==this.state.tabIndex} label={value}></TabComponents>
                            })
                        }
                    </View>
                ...
                <View style={style.footer}>
                    <Text onPress={()=>this.props.navigation.goBack()} style={style.goBack}>로그인 화면으로 돌아가기</Text>
                </View>
            </View>
        );
    }
